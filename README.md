...
ebx: [関係なし] #スタックに積むように一時的に使って他だけ →間違い？
ecx: 0xffffffe8 #重要そう。スタックのトップのアドレスをわざわざ入れてる
...
```
eax: 0x0000000b #超重要！システムコールexecveを示す番号11
ebx: [関係なし] #スタックに積むように一時的に使って他だけ→嘘
ecx: 0xffffffe8 #重要そう。execveの情報が入っているアドレスのトップ
edx: 0x00000000 #重要そう。わざわざedxをゼロにしているところがある。
esi: [関係なし]
edi: [関係なし]
ebp: [関係なし]
...
そして重要なのは書き込みのできるメモリ領域のアドレスを入手することか。
...
今回は`0x080490e0`になら行けそうな気がする。→書き込みはできた。
...
#dataaddr + 12に0x6e69622f (/sh )を書き込む
        mov eax, 0x0068732f
        mov edx, 0x080490e0+12
        mov [edx], eax
